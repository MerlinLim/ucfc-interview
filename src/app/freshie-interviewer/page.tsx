...
export default function Questionnaire() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-blue-100 no-print">
      <div className="max-w-4xl mx-auto p-6 text-sm md:text-base bg-white shadow-md rounded-lg print-container">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold">Find a senior to interview!</h1>
          <button
            onClick={handlePrint}
        ...
        <div className="grid grid-cols-1 gap-4 mb-8">
          {Object.entries(data["preUni"] as Record<string, string>).map(([label]) => (
            <div key={label}>
              <label className="block font-semibold">{label}:</label>
              <textarea className="w-full border border-gray-300 rounded p-2 mt-1" rows={2}></textarea>
            </div>
          ))}
        </div>
